...
function needsOffset( icon, size ) {
	var iconNeedsOffset = [
...
function needsOffsetX( icon, size ) {
...
function needsOffsetY( icon, size ) {
...
function gridicons( iconName, size, onClick ) {
...
/**
 * Uncomment this block and then use browserify
 * to create bundle.js with the latest changes and then
 * use test.html to test the gridicons
 */
document.getElementById( 'display-gridicons' ).innerHTML = gridicons( 'add-image', 24, "iconClickHandler" );
